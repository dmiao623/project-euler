...
namespace poly {
	namespace polynomial_internal {
		...
		std::mt19937 mst(std::chrono::steady_clock::now().time_since_epoch().count());
		...
		public:
			...
			template<int _mod>
			friend std::istream &operator>>(std::istream &is, Modular<_mod> &t);
		};

		template<int mod>
		std::istream &operator>>(std::istream &is, Modular<mod> &t) {
			int x;
			is >> x;
			t.value = x % mod;
			return is;
		...
		template<int mod>
		std::ostream &operator<<(std::ostream &os, const Modular<mod> &t) {
			return os << int(t);
		}

		template<typename T>
		T fact(int n) {
			static std::array<T, MAX_N> F;
		...
		namespace fast_fourier_transform {
			...
			void init_fast_fourier_transform() {
				...
				if (!init) {
					for (int i = 1; i < MAX_N; i *= 2) {
						...
						for (int j = 0; j < i; j++) {
							type ang = PI * j / i;
							w[i + j] = std::complex<type>(cos(ang), sin(ang));
							if (ti)
			...
			void slow_polynomial_multiply(std::vector<auto> &a, const std::vector<auto> &b) {
				if (a.empty() || b.empty()) {
					a.clear();
				} else {
					int n = a.size();
			...
			class discrete_fourier_transform {
				...
				std::vector<std::complex<type>> A;

			public:
				discrete_fourier_transform(std::vector<Modular<mod>> const &a, int n): A(n) {
					for (int i = 0; i < std::min(n, (int)a.size()); i++)
						A[i] = std::complex<type>(a[i].rem() % split, a[i].rem() / split);
					if (n)
				...
				auto operator*(discrete_fourier_transform const &B) {
					...
					int n = A.size();
					if (n == 0)
						return std::vector<Modular<mod>>();

					std::vector<std::complex<type>> C(n), D(n);
					for (int i = 0; i < n; i++) {
				...
				std::complex<type>& operator[](int i) { return A[i]; }

				std::complex<type> operator [](int i) const { return A[i]; }
			};
			...
			template<int mod>
			void polynomial_multiply(std::vector<Modular<mod>> &a, std::vector<Modular<mod>> b) {
				if (std::min(a.size(), b.size()) < MAX_B) {
	...
	public:
		...
		polynomial(const std::vector<T> &t) : coef(t) { normalize(); }
		...
		polynomial operator+=(const polynomial &t) {
			coef.resize(std::max(coef.size(), t.coef.size()));
			for (int i = 0; i < (int)t.coef.size(); i++)
				coef[i] += t.coef[i];
			normalize();
			return *this;
		}
		...
		polynomial modulo_k(int k) const { return std::vector<T>(coef.begin(), coef.begin() + std::min(k, (int)coef.size())); }
		...
		polynomial divide_k(int k) const {
			if (is_zero())
				return polynomial();
			return std::vector<T>(coef.begin() + std::min(k, (int)coef.size()), coef.end());
		}

		polynomial substr(int l, int r) const {
			return std::vector<T>(
				coef.begin() + std::min(l, (int)coef.size()),
				coef.begin() + std::min(r, (int)coef.size())
			);
		}
		...
		polynomial reverse(int n) const {
			auto res = coef;
			res.resize(std::max(n, (int)res.size()));
	...
	private:
		std::pair<polynomial, polynomial> slow_division(const polynomial &b) const {
			std::vector<T> A(coef);
			std::vector<T> res;
			while (A.size() >= b.coef.size()) {
				res.push_back(A.back() / b.coef.back());
				if (res.back() != T(0))
					for (int i = 0; i < (int)b.coef.size(); i++)
						A[(int)A.size() - i - 1] -= res.back() * b.coef[(int)b.coef.size() - i - 1];
				A.pop_back();
			}
	...
	public:
		std::pair<polynomial, polynomial> hint_division(polynomial const &b, polynomial const &binv) const {
			assert(!b.is_zero());
			if (deg() < b.deg())
				return {polynomial{0}, *this};
			int d = deg() - b.deg();
			if (std::min(d, b.deg()) < MAX_B)
				return slow_division(b);
			polynomial D = (reverse().modulo_k(d + 1) * binv.modulo_k(d + 1)).modulo_k(d + 1).reverse(d + 1);
			return {D, *this - D * b};
		}

		std::pair<polynomial, polynomial> division(const polynomial &b) const {
			assert(!b.is_zero());
			if (deg() < b.deg())
				return {polynomial{0}, *this};
			int d = deg() - b.deg();
	...
	private:
		struct transform {
			...
			auto apply(polynomial A, polynomial B) { return std::make_pair(a * A + b * B, c * A + d * B); }
		};

		template<typename Q>
		static void attatch(std::vector<Q> &a, std::vector<Q> const &b) {
			for (auto c : b)
				a.push_back(c);
		}

		static std::pair<std::vector<polynomial>, transform> half_gcd(polynomial A, polynomial B) {
			assert(A.deg() >= B.deg());
			int m = (A.deg() + 1) / 2;
			if (B.deg() < m)
		...
		static std::pair<std::vector<polynomial>, transform> full_gcd(polynomial A, polynomial B) {
			std::vector<polynomial> ak;
			...
			while (trs.size() >= 2) {
				trs[(int)trs.size() - 2] = trs[(int)trs.size() - 2] * trs[(int)trs.size() - 1];
				trs.pop_back();
			}
			return {ak, trs.back()};
		}
	...
	private:
		std::optional<polynomial> slow_modulo_inverse(polynomial const &t) const {
			auto R1 = *this, R2 = t;
			auto Q1 = polynomial(T(1)), Q2 = polynomial(T(0));
			int k = 0;
			while (!R2.is_zero()) {
	...
	public:
		std::optional<polynomial> modulo_inverse(polynomial const &t) const {
			assert(!t.is_zero());
			if (std::min(deg(), t.deg()) < MAX_B)
	...
	public:
		std::optional<polynomial> sqrt(int n) const {
			if (is_zero())
				return *this;
			int i = trailing_k();
			if (i % 2) {
	...
	private:
		...
		std::vector<T> chirpz_even(T z, int n) const {
			int m = deg();
			if (is_zero())
	...
	public:
		std::vector<T> chirpz(T z, int n) const {
	...
	private:
		std::vector<T> eval(std::vector<polynomial> &tree, int v, auto l, auto r) {
			if (r - l == 1) {
				return {eval(*l)};
			} else {
				auto m = l + (r - l) / 2;
	...
	private:
		polynomial interpolate(std::vector<polynomial> &tree, int v, auto l, auto r, auto ly, auto ry) {
			if (r - l == 1) {
				return {*ly / coef[0]};
			} else {
				auto m = l + (r - l) / 2;
		...
		static polynomial build(std::vector<polynomial> &res, int v, auto L, auto R) {
			if (R - L == 1) {
	...
	public:
		static auto interpolate(std::vector<T> x, std::vector<T> y) {
			int n = x.size();
			std::vector<polynomial> tree(4 * n);
		...
		static polynomial init_unit(int n) { return polynomial(std::vector<T>(n, 1)); }
	...
	private:
		polynomial eval_x2() {
			std::vector<T> res(2 * coef.size());
			for (int i = 0; i < (int)coef.size(); i++)
				res[2 * i] = coef[i];
			return res;
		}

		std::pair<polynomial, polynomial> bisect() const {
			std::vector<T> res[2];
	...
	public:
		...
		static polynomial compose_large(polynomial A, polynomial B, int n) {
			...
			int q = std::sqrt(n);
			auto [B0, B1] = std::make_pair(B.modulo_k(q), B.divide_k(q));
			B0 = B0.divide_k(1);
			std::vector<polynomial> pw(A.deg() + 1);
			auto getpow = [&](int k) { return pw[k].is_zero() ? pw[k] = B0.pow(k, n - k) : pw[k]; };

			std::function<polynomial(polynomial const&, int, int)> compose_dac = [&getpow, &compose_dac](polynomial const &f, int m, int N) {
				if (f.deg() <= 0)
					return f;
				int k = m / 2;
				auto [f0, f1] = std::make_pair(f.modulo_k(k), f.divide_k(k));
	...
	std::ostream &operator<<(std::ostream &os, const polynomial<auto> &p) {
		os << '[';
		if (p.size()) {
			os << p[0];
